...
/* To get struct ether_arp on Solaris, we include netinet/if_ether.h.
   ...
   So the result is that on Solaris 9, by including netinet/if_ether.h we get struct ether_arp,
   and don't get ether_aton(), ether_ntoa(), ether_ntohost(), ether_hostton(), but those four declarations
   aren't available in Solaris 9 anyway.  And in Solaris 10, we get struct ether_arp AND
